...
namespace keep.Pages
{
    public class LoginModel : PageModel
    {
        ...
        public void OnPost()
        {
            ...
            string keep_session_id = Guid.NewGuid().ToString();
            HttpContext.Response.Cookies.Append(
                bd_util.keep_SESSION_ID, keep_session_id, bd_util.get_cookie_options());

            string sql = @"insert into sessions (se_id, se_user)
                values (@se_id, @se_user)";

            var dict = new Dictionary<string, dynamic>();
            dict["@se_id"] = keep_session_id;
        ...
        public void Signout()
        {
            string session_id = HttpContext.Session.GetString(bd_util.keep_SESSION_ID);
            HttpContext.Session.SetString(bd_util.keep_SESSION_ID, "");
