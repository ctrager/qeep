...
cp keep_config_active.txt ../publish_next
cp git_log.txt ../publish_next
touch ../publish_next/$( date '+%Y-%m-%d_%H-%M-%S' )

# uncomment to reload db
#psql -d keep -U postgres -f sql/setup.sql
#psql -d keep -U postgres -f sql/queries.sql
#psql -d keep -U postgres -f sql/reports.sql
#psql -d keep -U postgres -f sql/generate_test_data.sql
